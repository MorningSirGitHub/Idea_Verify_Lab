...
namespace UnityEngine.UI
{
    ...
    public class HyperlinkText : Text, IPointerClickHandler
    {
        ...
        protected T LoadConfigAssets<T>(string path) where T : Object
        {
            ...
            var loadPath = "HyperlinkText/Emoji/" + EmojiType + type + path;
        ...
        protected T LoadAssets<T>(string path) where T : Object
        {
            ...
            if (Application.isEditor && !Application.isPlaying)
            {
                ...
                path = "Assets/Resources/" + path + extension;
            ...
            else
            {
                ...
                for (int i = 0; i < vertCount; ++i)
                {
                    ...
                    if (m_Emojis.TryGetValue(index, out info))
                    {
                        ...
                        {
                            // 如果是2019.1.5f1 及以上版本，顶点数据在单行状态下被优化掉了富文本占位符，所以这里再用取一遍数据
                            m_EmojisDic.TryGetValue(index, out info);
                            if (tempVertIndex == 3)
        ...
        protected void ParseEmoji()
        {
            //m_Text = Regex.Replace(m_Text, m_RegexEmoji, match => UnicodeCodePointToString(match.Value));
            m_Text = Regex.Replace(m_Text, m_RegexEmoji, match =>
            {
                var emojiString = match.Value;
                var unicodeName = new StringBuilder();
                unicodeName.Append("{");
                for (var i = 0; i < emojiString.Length; i += char.IsSurrogatePair(emojiString, i) ? 2 : 1)
                {
                    unicodeName.AppendFormat("{0:x4}-", char.ConvertToUtf32(emojiString, i));
                }
                unicodeName = unicodeName.Remove(unicodeName.Length - 1, 1);
                unicodeName.Append("}");
                return unicodeName.ToString();
            });
        }
        protected string UnicodeCodePointToString(string emojiString)
        {
            var unicodeName = new StringBuilder();
            unicodeName.Append("{");
            for (var i = 0; i < emojiString.Length; i += char.IsSurrogatePair(emojiString, i) ? 2 : 1)
